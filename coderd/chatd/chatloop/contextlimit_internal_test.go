...
func TestIsContextLimitKey(t *testing.T) {
	...
	tests := []struct {
		...
		// BUG: "max_context_version" matches the fallback heuristic
		// because it contains "context" and starts with "max". A
		// version field would be incorrectly treated as a context
		// limit key.
		{name: "max_context_version false positive", key: "max_context_version", want: true},
...
func TestPositiveInt64(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "positive", value: 1, want: 1, wantOK: true},
		{name: "large positive", value: 200000, want: 200000, wantOK: true},
		{name: "zero rejected", value: 0, want: 0, wantOK: false},
		{name: "negative rejected", value: -1, want: 0, wantOK: false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			got, ok := positiveInt64(tt.value)
			require.Equal(t, tt.wantOK, ok)
			require.Equal(t, tt.want, got)
		})
	}
