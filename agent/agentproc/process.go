...
// waitForOutput blocks until the buffer has received more
// than afterBytes total bytes, the buffer is closed (process
// exited), or the context is cancelled. Returns nil when
// state changed, ctx.Err() when the context expired.
func (p *process) waitForOutput(ctx context.Context, afterBytes int) error {
	p.buf.cond.L.Lock()
	...
	go func() {
		select {
		case <-ctx.Done():
			// Acquire-release the lock before broadcasting
			// to guarantee the waiter has entered
			// cond.Wait() (which atomically releases the
			// lock). Without this, a Broadcast between the
			// loop predicate check and cond.Wait() is lost.
			//nolint:gocritic // intentional lock-unlock-broadcast
			p.buf.cond.L.Lock()
			p.buf.cond.L.Unlock()
